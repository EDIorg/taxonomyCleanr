---
title: "Instructions for the taxonomyCleanr"
author: "Colin Smith"
date: "7/11/2018"
...
```{r instload devtools, echo=TRUE, message=FALSE, warning=FALSE}
# Install and load devtools
#install.packages('devtools')
library(devtools)
```

```{r instload taxonomyCleanr, echo=TRUE, message=FALSE, warning=FALSE}
...
### Load data
Load your data table into RStudio _as a data frame_. This data frame contains a single column of character strings representing your taxa. If you have species binomials in 2 columns, you will have to combine them into one.

```{r load data, echo=TRUE, paged.print=TRUE}
# Load test data
data <- read.table(paste0(path.package('taxonomyCleanr'), '/test_data.txt'), header = T, sep = '\t')
```

```{r view test data, echo=FALSE}
# View test data
knitr::kable(data, caption = "test data")
```
...
```{r view taxa_map.csv, echo=FALSE}
# View taxa_map.csv
knitr::kable(taxa_map, caption = "taxa_map.csv")
```
...
Notice some of the listed taxa (below) have conflicting spellings (e.g. "Achillea millefolium(lanulosa)" and "Achillea millefolium(lanulosaaaa)
"), and some of the taxa are clearly not taxa (e.g. "-9999" and "Miscellaneous litter").

```{r display unique 1, echo=FALSE}
# View taxa_map.csv
knitr::kable(output, caption = "Unique taxa and counts")
```
