---
title: "Using the taxonomyCleanr"
...
knitr::kable(head(output, n = 10), caption = "Unique taxa and counts after trim_taxa. Notice, extraneous characters (e.g. c.f., spp., and underscores) have been removed.")
```
...
# View unique taxa after trimming
knitr::kable(head(output, n = 10), caption = "Unique taxa counts after replacing misspelled taxa.")
```
...
knitr::kable(head(output, n = 10), caption = "Unique taxa and counts after non-taxa have been removed.")
```
...
knitr::kable(head(output, n = 10), caption = "Authorities supported by resolve_sci_taxa and resolve_comm_taxa")
```
...
# View authorities
output <- view_taxa_authorities()
knitr::kable(head(output, n = 10), caption = "Authorities supported by the resolve_sci_taxa and resolve_comm_taxa")
```

```{r resolve_common, message=FALSE, warning=FALSE}
# Resolve common using ITIS
resolve_comm_taxa(path = my_path, data.sources = 3)
```
...
knitr::kable(head(output, n = 10), caption = "taxa_map.csv after all the cleaning procedures have been applied.")
```
...
output <- read.table(paste0(my_path, '/taxonomyCleanr_output.txt'), header = T, sep = '\t', stringsAsFactors = F)
# View taxa_map.csv after running resolve_taxa
knitr::kable(head(output, n = 10), caption = "A revision of the raw data table with new taxonomic data appended")
```
